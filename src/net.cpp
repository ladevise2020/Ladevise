...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Ladevise " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"ladevisetools.com", "95.111.242.1"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Ladevise is probably already running."), addrBind.ToString().c_str());
        else
