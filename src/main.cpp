...
#if defined(NDEBUG)
# error "Ladevise cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x24c59d90d7291234d707fca302ca095bab9a071bda90817f5ccc98fb728c4437");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ladevise: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Ladevise Signed Message:\n";
...
{
    // Ladevise: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
   ...
   if (nHeight < 2000) {
    ...
    }else if(nHeight < 8002000){
    	nSubsidy = 100 * COIN;
    }

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    //nSubsidy >>= (nHeight / 840000); // Ladevise: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.5 * 24 * 60 * 60; // Ladevise: 12 hr
static const int64 nTargetSpacing = 2 * 60; // Ladevise: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ladevise: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ladevise: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x24c59d90d7291234d707fca302ca095bab9a071bda90817f5ccc98fb728c4437");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "LDC COIN BASE";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("756484710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1589589364;
        ...
        assert(block.hashMerkleRoot == uint256("0xebde0a562fbdef9d87866aa0440c42be80aeec0630cc2527a493311a0aac241f"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Ladevise: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LadeviseMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LadeviseMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LadeviseMiner(CWallet *pwallet)
{
    printf("LadeviseMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ladevise-miner");
    ...
    try { loop {
        ...
        printf("Running LadeviseMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LadeviseMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LadeviseMiner, pwallet));
}
